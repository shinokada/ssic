  ...
  for SUBSRC in "${CURRENTDIR}"/*; do
    ...
    for file in *; do
      ...
      if [ ! -f "${SUBDIRNAME}/${file}" ]; then
        cp "${script_dir}/src/awesome/v2/awesome-v2.txt" "${SVELTENAME}"
      fi
...
fn_awesome() {
  ...
  cp "${script_dir}/src/awesome/v2/Icon.svelte" "${CURRENTDIR}/Icon.svelte"
  cp "${script_dir}/src/awesome/v2/awesome-v2-types.txt" "${CURRENTDIR}/types.ts"
