  ...
  for SUBSRC in "${CURRENTDIR}"/*; do
    ...
    for file in *; do
      ...
      if [ ! -f "${CURRENTDIR}/${file}" ]; then
        cp "${script_dir}/templates/hero-v6.txt" "${CURRENTDIR}/${file}"
      fi
...
fn_modify_filenames() {
  ...
  for filename in "${CURRENTDIR}"/*.svg; do
    ...
    sed -i "s;replaceAriaLabel;${FILENAME};" "${filename}" >/dev/null 2>&1
  done
...
fn_hero() {
  ...
  DIRNAME='src'
  # SVGDIR='src'
  LOCAL_REPO_NAME="$HOME/Svelte/SVELTE-ICON-FAMILY/heros/svelte-heros"
