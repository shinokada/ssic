  ...
  for SUBSRC in "${DIR}/${SUBDIR}"/*; do
    ...
    for file in *; do
      ...
      if [ ! -f "${DIR}/${file}" ]; then
        ...
        cp "${script_dir}/src/heros2/v2/hero2-v2-template.txt" "${DIR}/${file}"
      fi
      # echo "${file}"
      SVGPATH=$(sed '1d; $d' "$file")
      # replace new line with space
...
fn_hero2() {
  ...
  cp "${script_dir}/src/heros2/v2/Icon.svelte" "${CURRENTDIR}/Icon.svelte"
  cp "${script_dir}/src/heros2/v2/heros2-types.txt" "${CURRENTDIR}/types.ts"
