fn_remix() {
  ...
  file_name="icons.js"
  repo_name="svelte-remix"

  clone_repo "$CURRENTDIR" "$DIRNAME" "$GITURL"

  # Use the find command to locate all SVG files in the subdirectories
  # and move them to the current directory
  find "$CURRENTDIR" -type f -name "*.svg" -exec mv {} "$CURRENTDIR" \;

  echo "removing all dirs"
  # Use the find command to locate all directories within CURRENTDIR
  # and remove them excluding files
  find "$CURRENTDIR" -mindepth 1 -type d -exec rm -r {} +
  echo "Completed removing"
  # Loop through all SVG files in the current directory
  for svg_file in *.svg; do
    # Extract the icon name and remove the 'ei-' prefix
    icon_name=$(extract_icon_name "$svg_file" "ei-")
