  ...
  for SUBSRC in "${CURRENTDIR}"/*; do
    ...
    if [[ $FILENAME =~ ^[0-9] ]]; then
      ...
      if [ ! -f "${SUBDIRNAME}/${file}" ]; then
        cp "${script_dir}/src/remix/remix-v2-template.txt" "${SVELTENAME}"
      fi
...
fn_remix() {
  ...
  DIRNAME='icons'
  LOCAL_REPO_NAME="$HOME/Svelte/SVELTE-ICON-FAMILY/Runes/svelte-remix-runes-webkit"
  ...
  cp "${script_dir}/src/remix/Icon.svelte" "${CURRENTDIR}/Icon.svelte"
  cp "${script_dir}/src/remix/remix-v2-types.txt" "${CURRENTDIR}/types.ts"
