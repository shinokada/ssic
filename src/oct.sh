fn_modify_svg() {
  ...
  sed -i '1s/^/<script lang="ts">export let size="16"; export let color="currentColor";<\/script>/' ./*.* && sed -i 's/viewBox=/fill={color} class={$$props.class} {...$$restProps} aria-label={ariaLabel} on:click on:keydown on:keyup on:focus on:blur on:mouseenter on:mouseleave on:mouseover on:mouseout &/' ./*.*
