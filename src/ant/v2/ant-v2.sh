  ...
  for SUBSRC in "${CURRENTDIR}"/*; do
    ...
    for file in *; do
      ...
      if [ ! -f "${SUBDIRNAME}/${file}" ]; then
        cp "${script_dir}/src/ant/v2/ant-v2-template.txt" "${SVELTENAME}"
      fi
...
#  modify file names
  ...
  cp "${script_dir}/src/ant/v2/Icon.svelte" "${CURRENTDIR}/Icon.svelte"
  cp "${script_dir}/src/ant/v2/ant-v2-types.txt" "${CURRENTDIR}/types.ts"
