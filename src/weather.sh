fn_weather() {
  ...
  for svg_file in *.svg; do
    ...
    icon_name=$(extract_icon_name "$svg_file" "wi-")
    ...
    if [ -n "$path_data" ]; then
      ...
      if [ -f "$file_name" ]; then
        ...
        new_entry=", '$icon_name': { box: 30, svg: '$path_data' }"
        ...
        echo "{ '$icon_name': { box: 30, svg: '$path_data' }, }" > "$file_name"
      fi
  ...
  # replace replace_size with 30
  target_value="\"30\""
  sed -i "s/replace_size/$target_value/g" Icon.svelte
  # replace replace_name with svelte-weather
  sed -i "s/replace_name/svelte-weather/g" Icon.svelte
